...
static int aml_asoc_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	pr_info("enter %s\n", __func__);
	p_aml_audio = snd_soc_card_get_drvdata(card);
	ret = snd_soc_add_card_controls(card, aml_m8_controls,
	...
	if (!p_aml_audio->hp_disable) {
		...
		ret =
		    snd_soc_card_jack_new(card, "hp switch", SND_JACK_HEADPHONE,
				     &p_aml_audio->jack, jack_pins,
...
static void aml_m8_pinmux_init(struct snd_soc_card *card)
{
	...
	p_aml_audio->mute_desc = gpiod_get(card->dev, "mute_gpio", 0);
